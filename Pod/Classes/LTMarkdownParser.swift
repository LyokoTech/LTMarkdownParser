...
open class LTMarkdownParser: TSBaseParser {
	...
	open var headerAttributes = [[String: AnyObject]]()
	open var listAttributes = [[String: AnyObject]]()
	open var numberedListAttributes = [[String: AnyObject]]()
	open var quoteAttributes = [[String: AnyObject]]()

	open var imageAttributes = [String: AnyObject]()
	open var linkAttributes = [String: AnyObject]()
	open var monospaceAttributes = [String: AnyObject]()
	open var strongAttributes = [String: AnyObject]()
	open var emphasisAttributes = [String: AnyObject]()
	open var strongAndEmphasisAttributes = [String: AnyObject]()

	open static var standardParser = LTMarkdownParser()

	class func addAttributes(_ attributesArray: [[String: AnyObject]], atIndex level: Int, toString attributedString: NSMutableAttributedString, range: NSRange) {
	...
	open func addLinkParsingWithFormattingBlock(_ formattingBlock: @escaping LTMarkdownParserFormattingBlock) {
		...
		addParsingRuleWithRegularExpression(linkRegex) { match, attributedString in
			...
			var urlCharacterSet = CharacterSet(charactersIn: "")
			urlCharacterSet.formUnion(CharacterSet.urlPathAllowed)
			urlCharacterSet.formUnion(CharacterSet.urlPathAllowed)
			urlCharacterSet.formUnion(CharacterSet.urlQueryAllowed)
			urlCharacterSet.formUnion(CharacterSet.urlFragmentAllowed)


			if let URL = URL(string: linkURLString) ?? URL(string: linkURLString.addingPercentEncoding(withAllowedCharacters: urlCharacterSet) ?? linkURLString) {
